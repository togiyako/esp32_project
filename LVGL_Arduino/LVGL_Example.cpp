...
void Lvgl_Example1(void) {
  ...
  lv_style_set_text_font(&style_label, font_large);
  lv_style_set_text_align(&style_label, LV_ALIGN_CENTER);
  lv_style_set_text_color(&style_label, LV_COLOR_MAKE(0, 0, 0));

  lv_style_t style_bg;
  lv_style_init(&style_bg);
  lv_style_set_bg_color(&style_bg, LV_COLOR_MAKE(255, 255, 255));

  lv_obj_add_style(lv_scr_act(), &style_bg, 90);
  ...
  char buf[32];
  snprintf(buf, sizeof(buf), "%d", random_number);
